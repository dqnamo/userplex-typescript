...
export class Userplex {
   ...
   * @param {string} [opts.baseURL=process.env['USERPLEX_BASE_URL'] ?? https://api.userplex.app] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('USERPLEX_BASE_URL'),
    apiKey = readEnv('USERPLEX_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.UserplexError(
        "The USERPLEX_API_KEY environment variable is missing or empty; either provide it, or instantiate the Userplex client with an apiKey option, like new Userplex({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://api.userplex.app`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Userplex.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('USERPLEX_LOG'), "process.env['USERPLEX_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.userplex.app';
  }
  ...
  static Userplex = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static UserplexError = Errors.UserplexError;
